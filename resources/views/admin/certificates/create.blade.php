    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.certificates.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('awarded_by') ? 'is-invalid' : '' }}" type="text" name="awarded_by" id="awarded_by" value="{{ old('awarded_by', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_printed') ? 'is-invalid' : '' }}" type="text" name="date_printed" id="date_printed" value="{{ old('date_printed') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('recipient_name') ? 'is-invalid' : '' }}" name="recipient_name_id" id="recipient_name_id" required>
                    @foreach($recipient_names as $id => $entry)
                        <option value="{{ $id }}" {{ old('recipient_name_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
