...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.membershipsCorporate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.memberships-corporates.update", [$membershipsCorporate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('statuses') ? 'is-invalid' : '' }}" name="statuses[]" id="statuses" multiple>
                    @foreach($statuses as $id => $status)
                        <option value="{{ $id }}" {{ (in_array($id, old('statuses', [])) || $membershipsCorporate->statuses->contains($id)) ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('application_no') ? 'is-invalid' : '' }}" name="application_no_id" id="application_no_id">
                    @foreach($application_nos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('application_no_id') ? old('application_no_id') : $membershipsCorporate->application_no->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('member_class') ? 'is-invalid' : '' }}" name="member_class_id" id="member_class_id" required>
                    @foreach($member_classes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('member_class_id') ? old('member_class_id') : $membershipsCorporate->member_class->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('valid_from') ? 'is-invalid' : '' }}" type="text" name="valid_from" id="valid_from" value="{{ old('valid_from', $membershipsCorporate->valid_from) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('valid_till') ? 'is-invalid' : '' }}" type="text" name="valid_till" id="valid_till" value="{{ old('valid_till', $membershipsCorporate->valid_till) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('member_no') ? 'is-invalid' : '' }}" type="text" name="member_no" id="member_no" value="{{ old('member_no', $membershipsCorporate->member_no) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company_name') ? 'is-invalid' : '' }}" name="company_name_id" id="company_name_id" required>
                    @foreach($company_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('company_name_id') ? old('company_name_id') : $membershipsCorporate->company_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('training_credits') ? 'is-invalid' : '' }}" type="number" name="training_credits" id="training_credits" value="{{ old('training_credits', $membershipsCorporate->training_credits) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('support_funds') ? 'is-invalid' : '' }}" name="support_funds[]" id="support_funds" multiple>
                    @foreach($support_funds as $id => $support_fund)
                        <option value="{{ $id }}" {{ (in_array($id, old('support_funds', [])) || $membershipsCorporate->support_funds->contains($id)) ? 'selected' : '' }}>{{ $support_fund }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('admission_date') ? 'is-invalid' : '' }}" type="text" name="admission_date" id="admission_date" value="{{ old('admission_date', $membershipsCorporate->admission_date) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $membershipsCorporate->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
