...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.paymentSource.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.payment-sources.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="payment_source_name">{{ trans('cruds.paymentSource.fields.payment_source_name') }}</label>
                <input class="form-control {{ $errors->has('payment_source_name') ? 'is-invalid' : '' }}" type="text" name="payment_source_name" id="payment_source_name" value="{{ old('payment_source_name', '') }}" required>
