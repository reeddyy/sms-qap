...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.paymentsEdp.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments-edps.update", [$paymentsEdp->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('admission_no') ? 'is-invalid' : '' }}" name="admission_no_id" id="admission_no_id" required>
                    @foreach($admission_nos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('admission_no_id') ? old('admission_no_id') : $paymentsEdp->admission_no->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_source') ? 'is-invalid' : '' }}" name="payment_source_id" id="payment_source_id" required>
                    @foreach($payment_sources as $id => $entry)
                        <option value="{{ $id }}" {{ (old('payment_source_id') ? old('payment_source_id') : $paymentsEdp->payment_source->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_amount') ? 'is-invalid' : '' }}" type="number" name="payment_amount" id="payment_amount" value="{{ old('payment_amount', $paymentsEdp->payment_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('payment_date') ? 'is-invalid' : '' }}" type="text" name="payment_date" id="payment_date" value="{{ old('payment_date', $paymentsEdp->payment_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('invoice_no') ? 'is-invalid' : '' }}" type="text" name="invoice_no" id="invoice_no" value="{{ old('invoice_no', $paymentsEdp->invoice_no) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $paymentsEdp->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
