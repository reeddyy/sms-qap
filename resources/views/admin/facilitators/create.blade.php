    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.facilitators.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('facilitator_title') ? 'is-invalid' : '' }}" name="facilitator_title" id="facilitator_title">
                    ...
                    @foreach(App\Models\Facilitator::FACILITATOR_TITLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('facilitator_title', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('facilitator_name') ? 'is-invalid' : '' }}" type="text" name="facilitator_name" id="facilitator_name" value="{{ old('facilitator_name', '') }}" required>
