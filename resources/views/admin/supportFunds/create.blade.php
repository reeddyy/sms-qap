...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.supportFund.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.support-funds.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="fund_name">{{ trans('cruds.supportFund.fields.fund_name') }}</label>
                <input class="form-control {{ $errors->has('fund_name') ? 'is-invalid' : '' }}" type="text" name="fund_name" id="fund_name" value="{{ old('fund_name', '') }}" required>
