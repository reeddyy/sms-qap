...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.memberClass.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.member-classes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="member_class_name">{{ trans('cruds.memberClass.fields.member_class_name') }}</label>
                <input class="form-control {{ $errors->has('member_class_name') ? 'is-invalid' : '' }}" type="text" name="member_class_name" id="member_class_name" value="{{ old('member_class_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\MemberClass::MEMBER_CATEGORY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('member_category') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="member_category_{{ $key }}" name="member_category" value="{{ $key }}" {{ old('member_category', '') === (string) $key ? 'checked' : '' }}>
