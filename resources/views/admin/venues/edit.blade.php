    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.venues.update", [$venue->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('venue_name') ? 'is-invalid' : '' }}" type="text" name="venue_name" id="venue_name" value="{{ old('venue_name', $venue->venue_name) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $venue->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
