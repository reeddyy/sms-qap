    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.programmes.update", [$programme->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('edp_title') ? 'is-invalid' : '' }}" type="text" name="edp_title" id="edp_title" value="{{ old('edp_title', $programme->edp_title) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('edp_abbr') ? 'is-invalid' : '' }}" type="text" name="edp_abbr" id="edp_abbr" value="{{ old('edp_abbr', $programme->edp_abbr) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duration_day') ? 'is-invalid' : '' }}" type="text" name="duration_day" id="duration_day" value="{{ old('duration_day', $programme->duration_day) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('level') ? 'is-invalid' : '' }}" name="level_id" id="level_id">
                    @foreach($levels as $id => $entry)
                        <option value="{{ $id }}" {{ (old('level_id') ? old('level_id') : $programme->level->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('level'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('member_rate') ? 'is-invalid' : '' }}" type="number" name="member_rate" id="member_rate" value="{{ old('member_rate', $programme->member_rate) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('public_rate') ? 'is-invalid' : '' }}" type="number" name="public_rate" id="public_rate" value="{{ old('public_rate', $programme->public_rate) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Programme::EDP_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('edp_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="edp_status_{{ $key }}" name="edp_status" value="{{ $key }}" {{ old('edp_status', $programme->edp_status) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $programme->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
