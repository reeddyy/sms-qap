    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.grades.update", [$grade->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('grade_letter') ? 'is-invalid' : '' }}" type="text" name="grade_letter" id="grade_letter" value="{{ old('grade_letter', $grade->grade_letter) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('grade_point_s') ? 'is-invalid' : '' }}" type="number" name="grade_point_s" id="grade_point_s" value="{{ old('grade_point_s', $grade->grade_point_s) }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $grade->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
