...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.applicantsAda.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.applicants-adas.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ada_status') ? 'is-invalid' : '' }}" name="ada_status_id" id="ada_status_id" required>
                    @foreach($ada_statuses as $id => $entry)
                        <option value="{{ $id }}" {{ old('ada_status_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('award_name') ? 'is-invalid' : '' }}" name="award_name_id" id="award_name_id" required>
                    @foreach($award_names as $id => $entry)
                        <option value="{{ $id }}" {{ old('award_name_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('validity_start') ? 'is-invalid' : '' }}" type="text" name="validity_start" id="validity_start" value="{{ old('validity_start') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('validity_end') ? 'is-invalid' : '' }}" type="text" name="validity_end" id="validity_end" value="{{ old('validity_end') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ada_no') ? 'is-invalid' : '' }}" type="text" name="ada_no" id="ada_no" value="{{ old('ada_no', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('applicant_name') ? 'is-invalid' : '' }}" name="applicant_name_id" id="applicant_name_id" required>
                    @foreach($applicant_names as $id => $entry)
                        <option value="{{ $id }}" {{ old('applicant_name_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
